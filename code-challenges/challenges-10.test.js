...
const createServer = () => {
  ...
  app.get('/hello',(request,response)=>{
    response.status(200).send('Hi this is hello route');
  });
  ...
  function aboutMeHandler(request,response){
    response.status(200).send('This is about me route, here is my life etc');
  }

  function favoriteFoodsHandler(request,response){
    response.status(200).send(['ice','cream','avocado','fish']);
  }

  function catchAllHandler(request,response){
    response.status(404).send('Something went wrong, Not Found');
  }
